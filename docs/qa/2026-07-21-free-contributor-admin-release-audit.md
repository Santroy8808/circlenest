...
| A-010 | Moderation | Review reports, restrictions, held posts, and moderation audit records | Partial repair complete; production retest pending | Conduct report search, assignment, evidence/dispute context, legal versioned transitions, and atomic audit receipts are repaired in code. Restriction and held-post coverage plus broader audit-history depth remain separate checks. See `ADM-003`, `ADM-012`, `ADM-014`, and `LIVE-016`. |
...
| `ADM-003` / `LIVE-016` / report portion of `A-010` | Repaired in code | Bounded report query, reviewer assignment, legal versioned transitions, idempotent receipts, stable locking, actor reauthorization, linked-dispute safeguards, 49 focused conduct tests, lint/type checks, and production build | Deploy and repeat report search, assignment, transition, replay, and two-admin stale-version conflict workflows in production; restriction/hold coverage remains separate |
...
| ADM-003 | S2 | A-010 | Historical production result: Reports Queue was read-only. Repair status: the report-review portion is fixed in code with bounded search, assignment, legal versioned status actions, reason/note capture, and durable audit receipts; production retest is pending. Restrictions and holds remain separate tools/checks. | The old surface fetched a report slice without an operational mutation contract. The replacement uses one typed UI/API transition policy and atomic service commands. | Deploy and exercise report search, assignment, legal transitions, duplicate replay, and stale-version conflict. Track restriction and hold operations independently rather than implying the report page performs them. |
...
| LIVE-016 | S2 | A-010 | Historical production result: Reports Queue loaded as an empty read-only list. Repair status: the replacement Conduct Report Review workspace implements bounded report search, assignment, evidence/dispute context, and audited legal status transitions; production confirmation is pending. | The former page lacked a shared UI/API transition contract and atomic command service. | Deploy and repeat the report workspace matrix. Verify restriction and post-hold tools separately; do not present them as report-page actions. |
...
1. The gallery/avatar defect is repaired in code but still requires its post-deployment production confirmation. Free also has a public-versus-members Stream contract conflict, the wrong listing allowance, missing profile posts, forbidden Stream filters, and severe narrow-viewport overflow.
