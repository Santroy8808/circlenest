...
const settingShortcuts: SettingsSearchEntry[] = [
  ...
  {
    title: "Rules > Notification Dings",
...
export function SettingsHub({ cards }: { cards: SettingsCard[] }) {
  ...
  const entries = useMemo<SettingsSearchEntry[]>(() => {
    ...
    [...settingShortcuts, ...cardEntries].forEach((entry) => {
      ...
      <section className="surface rounded-md p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold">{showingSearch ? `${visibleEntries.length} matching setting${visibleEntries.length === 1 ? "" : "s"}` : "Choose an area"}</h2>
          </div>
          {showingSearch ? (
