...
function emptyDraft(templates: Record<MarketplaceListingKind, MarketplaceTemplateDefinition>, publisherKey: string, initialIntent: "OFFER" | "WANTED" = "OFFER"): WizardDraft {
  return {
    ...
    countryCode: "US", region: "", city: "", postalArea: "", exactAddress: "", remote: false, deliveryAvailable: false,
...
function existingDraft(listing: MarketplaceListingDetailView, publisherKey: string): WizardDraft {
  ...
  return {
    kind: listing.kind, intent: listing.intent, title: listing.title, summary: listing.summary ?? "", description: listing.description, category: listing.category, subcategory: listing.subcategory ?? "", condition: listing.condition ?? "", attributes: listing.attributes as Record<string, unknown>,
...
export function MarketplaceListingWizard({
  ...
  function setKind(kind: MarketplaceListingKind) { patch({ kind, category: templates[kind].categories[0] ?? "Other", attributes: {}, priceType: kind === "JOB" ? "RANGE" : kind === "SERVICE" || kind === "AUDITOR" ? "QUOTE" : "FIXED" }); }
  ...
  function validateStep(currentStep: number) {
    ...
    if (currentStep === 2 && !draft.remote && draft.countryCode.trim().length !== 2) return "Enter a two-letter country code or mark the listing remote.";
    ...
    startTransition(async () => {
      ...
      try {
        ...
        const listing: MarketplaceListingInput = {
          ...
          location: { countryCode: draft.countryCode || null, region: draft.region || null, city: draft.city || null, postalArea: draft.postalArea || null, exactAddress: draft.exactAddress || null, remote: draft.remote, deliveryAvailable: draft.deliveryAvailable },
