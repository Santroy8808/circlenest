  ...
  return (
    <div className="space-y-4">
      <section className="card space-y-3 p-4">
        <div>
          <h2 className="text-lg font-semibold">My Writers Studio</h2>
          <p className="text-xs text-slate-400">
            {canCreate ? "Create a writing project and first article here." : accessReason ?? "Browse only."}
          </p>
        </div>

        <div className="grid gap-2 md:grid-cols-2">
          <input value={title} onChange={(event) => setTitle(event.target.value)} disabled={!canCreate} placeholder="Project title" className="rounded border border-slate-300 px-3 py-2 text-sm disabled:cursor-not-allowed disabled:bg-slate-100" />
          ...
          <input value={articleTitle} onChange={(event) => setArticleTitle(event.target.value)} disabled={!canCreate} placeholder="First article title" className="rounded border border-slate-300 px-3 py-2 text-sm disabled:cursor-not-allowed disabled:bg-slate-100" />
          <textarea value={summary} onChange={(event) => setSummary(event.target.value)} disabled={!canCreate} placeholder="Project summary" className="min-h-24 rounded border border-slate-300 px-3 py-2 text-sm disabled:cursor-not-allowed disabled:bg-slate-100 md:col-span-2" />
          <textarea value={articleBody} onChange={(event) => setArticleBody(event.target.value)} disabled={!canCreate} placeholder="First article body" className="min-h-32 rounded border border-slate-300 px-3 py-2 text-sm disabled:cursor-not-allowed disabled:bg-slate-100 md:col-span-2" />
        </div>
      ...
      <section className="card space-y-3 p-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Your own writing projects and articles.</p>
        </div>
      ...
      <section className="card space-y-3 p-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Browse public Writers Studio projects.</p>
        </div>
        <div className="space-y-2">
              ...
              <article key={project.id} className="rounded border border-[var(--border)] p-3">
                ...
                <div className="mt-2 space-y-2">
                  {project.articles.length ? (
                    ...
                    <p className="text-xs text-slate-500">No public articles yet.</p>
                  )}
                </div>
              </article>
            ))
