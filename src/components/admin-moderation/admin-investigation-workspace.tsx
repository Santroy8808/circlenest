            ...
            <div className="mt-4 grid gap-4">
              ...
              {workspace?.investigations.map((investigation) => {
                ...
                return <article className="rounded-md border border-[var(--line)] p-4" key={investigation.id}>
                  ...
                  {investigation.sourcePostIds.length ? <details className="mt-4 rounded-md border border-[var(--line)] p-3"><summary className="cursor-pointer font-semibold text-[var(--gold)]">Complete source package ({investigation.sourcePostIds.length})</summary><div className="mt-3 flex flex-wrap gap-2">{investigation.sourcePostIds.map((postId) => <Link className="text-sm font-semibold text-[var(--gold)] hover:underline" href={`/posts/${postId}`} key={postId}>Source {postId.slice(0, 8)}</Link>)}</div></details> : null}
                </article>;
              })}
            </div>
          </section>
