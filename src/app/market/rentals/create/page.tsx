...
export default async function CreateMarketRentalPage() {
  const session = await auth();
  if (!session?.user || session.user.revoked) redirect("/login?callbackUrl=/market/rentals/create");
  ...
  return (
    <AppShell>
      <CreateMarketListingForm createState={createState} listingKind="rental" />
    </AppShell>
  );
}
