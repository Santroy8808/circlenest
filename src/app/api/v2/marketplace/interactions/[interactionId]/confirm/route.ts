...
import { confirmMarketplaceInteraction } from "@/modules/marketplace/marketplace-interactions.service";
...
export async function POST(_request: NextRequest, context: { params: Promise<{ interactionId: string }> }) {
  ...
  return marketplaceResult(await confirmMarketplaceInteraction(user.id, (await context.params).interactionId));
}
