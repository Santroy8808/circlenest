...
import {
  ...
  respondToFriendRelationshipRequest
...
export async function POST(request: NextRequest) {
  ...
  const result = await requestFriendRelationship(session.user.id, body.value);
...
export async function PATCH(request: NextRequest) {
  ...
  const result = await respondToFriendRelationshipRequest(session.user.id, requestId, { action: body.action });
  if (!result.ok) return NextResponse.json({ error: result.error }, { status: 400 });
